...
class Student {
    ...
    constructor(registeredNumber, name, email, password) {
    ...
    #registerStudent(student) {
        const [name,registeredNumber,email,password]=[student.name,student.registeredNumber,student.email,student.password];
        showMessage("Registering your account, please wait...", STATUS.INFO);
        fetch("register", {
            method: "POST",
            headers: {"Content-Type": "application/x-www-form-urlencoded"},
            body: `name=${encodeURIComponent(name)}&registeredNumber=${encodeURIComponent(registeredNumber)}&email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`
        })
            .then((res) => res.text())
            .then((data) => {
                if (data === "exists") {
...
if(window.location.pathname.endsWith("register.html")) {
    document.addEventListener("DOMContentLoaded", () => {
            document.getElementById("registerForm").addEventListener("submit", (e) => {
                ...
                new Student(reg, name, email, pass);
            });
        }
    );
}